...
(async()=>{
  ...
  try{
    ...
    for(const size of [{width:320,height:568},{width:390,height:740},{width:430,height:820},{width:844,height:390}]){
      ...
      if(await page.locator('#sp-library-categories-toggle').getAttribute('aria-expanded')!=='true')await page.locator('#sp-library-categories-toggle').tap();await page.locator('[data-category="stage"]').tap();
      ...
      await page.locator('#sp-library-search').fill('');await page.locator('#sp-library-search').blur();
      await page.locator('#sp-library-categories-toggle').waitFor({state:'visible'});if(await page.locator('#sp-library-categories-toggle').getAttribute('aria-expanded')!=='true')await page.locator('#sp-library-categories-toggle').tap();
    ...
    await page.locator('#sp-library-search').fill('');await page.locator('#sp-library-search').blur();
    await page.locator('[data-category="guitars"]').click();
    await page.locator('[data-library-model-family="electric-basses"]').click();
    const modelDialog=page.locator('#sp-model-dialog');await modelDialog.waitFor({state:'visible'});
    const paletteBox=await page.locator('#sp-library').boundingBox(),dialogBox=await modelDialog.boundingBox();
    assert(dialogBox.x>=paletteBox.x+paletteBox.width,'Model picker leaves the category navigation visible');
    assert.equal(await modelDialog.evaluate(el=>el.matches(':modal')),false,'Desktop model selection is nonmodal');
    assert.equal(await modelDialog.locator('[data-dialog-model]').count(),9);
    const modelStar=modelDialog.locator('[data-model-favorite="bass-rickenbacker"]');await modelStar.click();
    assert.equal(await modelStar.getAttribute('aria-pressed'),'true');assert(await modelDialog.isVisible());
    await modelStar.press('Escape');await modelDialog.waitFor({state:'hidden'});
    await page.locator('#sp-library-search').fill('Mustang');
    assert.equal(await page.locator('#sp-library-items [data-add="bass-mustang"]').count(),1);
    assert.equal(await page.locator('#sp-library-items [data-library-model-family]').count(),0);
    await page.locator('#sp-library-search').fill('');await page.locator('[data-library-scope="favorites"]').click();
    assert.equal(await page.locator('#sp-library-items [data-add="bass-rickenbacker"]').count(),1);
