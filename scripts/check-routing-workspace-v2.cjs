...
async function checkSourceOutputs(browser,errors){
 ...
 try{
  ...
  await page.locator('#sp-inspector-output-pairs [data-inspector-output]').first().click();assert(await workspace.isVisible());assert(await count.isVisible());assert.equal(await count.inputValue(),'2','The inspector shortcut opens this source and its output controls directly');
  await page.evaluate(()=>{
