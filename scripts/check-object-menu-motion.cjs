...
(async()=>{
 ...
 try{for(const width of [1280,390]){
  ...
  const center=async()=>{const b=await floor.locator('[data-object="keys-motion"]').boundingBox();return{x:b.x+b.width/2,y:b.y+b.height/2};};
  let point=await center();await page.mouse.move(point.x,point.y);await page.mouse.down();
  assert.equal(await menu.isVisible(),false,'Do not animate before the selection redraw on pointerup.');
  assert.equal(await page.evaluate(()=>window.menuEntrances.length),0);
  await page.mouse.up();await menu.waitFor({state:'visible'});
  ...
  await page.emulateMedia({reducedMotion:'reduce'});point=await center();await page.mouse.click(point.x,point.y);await menu.waitFor({state:'visible'});
