 ...
 try{
  ...
  await context.route('**/stageplot-assets/vendor/supabase.js',route=>route.fulfill({contentType:'text/javascript',body:`
    ...
    window.StageplotSupabase={createClient:(_url,_key,options)=>{
      const key=options.auth.storageKey;if(!key.startsWith('stageplot-share-owner-'))throw new Error('Email accounts must remain disabled');
      return {auth:{getSession:async()=>({data:{session:JSON.parse(localStorage.getItem(key)||'null')}}),signInAnonymously:async()=>{
        window.qaAnonymousCreations++;const session={user:{id:'browser-owner',is_anonymous:true},access_token:'browser-token'};localStorage.setItem(key,JSON.stringify(session));return {data:{session}};
      }}};
    }};`}));
  ...
  await context.route(host+'/rest/v1/rpc/**',async route=>{
   ...
   const result=action==='get'?(stored.get(id)||null):{active:stored.has(id),owned:stored.has(id)&&token==='Bearer browser-token',available:!stored.has(id),updated_at:stored.get(id)?.updated_at||null};
