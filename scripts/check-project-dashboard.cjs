...
(async()=>{const browser=await(engine==='webkit'?webkit.launch({headless:true}):chromium.launch({channel:'chrome',headless:true}));try{
 ...
 await p.screenshot({path:'/private/tmp/project-dashboard-'+engine+'.png'});
 ...
 await p.locator('#sp-prototype').evaluate(el=>el.dataset.theme='dark');await p.screenshot({path:'/private/tmp/project-dashboard-dark-'+engine+'.png'});
 ...
 await p.screenshot({path:'/private/tmp/project-dashboard-long-'+engine+'.png'});
