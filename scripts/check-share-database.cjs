...
(async()=>{
 ...
 try{
  ...
  for(const file of ['0001_stageplot_documents.sql','0002_inventory.sql','0003_project_shares.sql','0004_reusable_share_ids.sql','0005_share_only_guest_access.sql','0006_share_label_positions.sql','0007_instrument_details.sql'])await db.exec(fs.readFileSync('supabase/migrations/'+file,'utf8'));
  // Sharing migration may safely be applied again.
  await db.exec(fs.readFileSync('supabase/migrations/0007_instrument_details.sql','utf8'));
