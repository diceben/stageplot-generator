...
(async()=>{
 ...
 try{
  ...
  for(const file of ['0001_stageplot_documents.sql','0002_inventory.sql','0003_project_shares.sql'])await db.exec(fs.readFileSync('supabase/migrations/'+file,'utf8'));
  // Sharing migration may safely be applied again.
  await db.exec(fs.readFileSync('supabase/migrations/0003_project_shares.sql','utf8'));
  const role=async(name,user='')=>{await db.exec('reset role');await db.query("select set_config('request.jwt.claim.sub',$1,false)",[user]);await db.exec('set role '+name);};
  ...
  await role('anon');assert.equal(await call('get','SP-ONE'),null);
  for(const action of ['publish','revoke','status'])await assert.rejects(call(action,'SP-ONE',action==='publish'?doc('SP-ONE'):undefined),/permission denied/);
  ...
  for(const action of ['publish','revoke','status'])await assert.rejects(call(action,'SP-ONE',action==='publish'?doc('SP-ONE'):undefined),/NOT_OWNER/);
  ...
  await role('authenticated',other);await assert.rejects(call('publish','SP-ONE',doc('SP-ONE')),/NOT_OWNER/);
  ...
  await db.exec('reset role');const row=(await db.query("select document,revoked_at from public.stageplot_project_shares where project_id='SP-ONE'")).rows[0];assert.deepEqual(row.document,{});assert.ok(row.revoked_at);
  console.log('PASS PostgreSQL: migrations, role grants, no listing, owner isolation, recursive sanitization, revocation, ID reservation and quota.');
