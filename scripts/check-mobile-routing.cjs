...
(async()=>{const browser=await launchBrowser();try{
 ...
 await page.screenshot({path:artifactPath('routing-before-check-'+engine+'.png')});
 ...
 await page.locator('#sp-stagebox-tab').tap();assert(await page.locator('#sp-stagebox-view').isVisible());await page.screenshot({path:artifactPath('routing-stageboxes-'+engine+'.png')});
 ...
 await page.setViewportSize({width:390,height:670});await page.locator('.sp-routing-table-wrap').evaluate(el=>el.scrollTop=0);await page.screenshot({path:artifactPath('routing-mobile-'+engine+'.png')});
 await page.locator('#sp-routing-tools-open').tap();await page.screenshot({path:artifactPath('routing-actions-'+engine+'.png')});await page.locator('#sp-routing-tools-close').tap();
