...
**Aktuelle Version:** v0.1.0-beta.30 · 18. September 2026. Release Notes sind in der App über `?` erreichbar.
...
Die DI-Modellwahl öffnet direkt am DI-Baustein in der Bibliothek wie bei den Gitarren. Sieben gleich große Modellkacheln, Suche und Modellfavoriten stehen vor dem Platzieren bereit. Die Eigenschaften zeigen die gewählte Box und ihre Anschlüsse. Jede DI im Routing steht auch als verschiebbare Box auf der Bühne. Neue Boxen werden neben der Quelle platziert; manuell aufgestellte Boxen lassen sich über **Box zuordnen** anschließen. Stereo nutzt eine gemeinsame Box. Modellwechsel erhalten die Position, und ein Klick auf die Bühnenbox zeigt ihre Eingänge. Ältere lokale Pläne übernehmen vorhandene Geräte, Kanalnummern und Stagebox-Zuordnungen.

Links erscheint **Stagebox** als ein gemeinsamer Eintrag, auch in Suche, Favoriten und zuletzt verwendeten Objekten. Ein Klick öffnet die Modellauswahl. Darin stehen zehn Modelle von Behringer und Allen & Heath mit generierten Gerätebildern zur Wahl: S16, S32, SD8, SD16, AR84, AR2412, AB168, DX168, DT168 und GX4816. Dieselben Bilder zeigen im Routing antippbare Ein- und Ausgänge an den tatsächlichen Buchsenpositionen. Stereo-Paare, Pultkanalnummern und vorhandene lokale Pläne bleiben erhalten. Modellwechsel schützen belegte Anschlüsse. [Bilder und Herstellerreferenzen](stageplot-assets/stageboxes/manifest.json), [Generierungsprompts](stageplot-assets/stageboxes/prompts-v1.json).

Die Online-Freigabe benötigt die Einrichtung aus [ACCOUNT_SETUP.md](ACCOUNT_SETUP.md).

## Entwicklung und Prüfungen

`npm run check` prüft alle Laufzeitmodule und eingebetteten Skripte. `npm test` prüft den Build-Abgleich und die 50 Funktionstestgruppen. Für Browserprüfungen einmal `npm ci` und `npx playwright install chromium webkit` ausführen, danach `npm run test:browser` und `BROWSER=webkit npm run test:browser`. Ohne `APP_URL` startet und beendet der Lauf seinen eigenen lokalen Vorschau-Server. Screenshots und die PDF-Gegenprobe mit Schlagzeug und Jazzmaster liegen in `test-results/`. GitHub Actions prüft Chromium und WebKit; beide müssen vor dem Deployment erfolgreich sein. Details: [Mobile QA](MOBILE_QA.md).

## Funktionen

- **Bassmodelle:** J-Bass, P-Bass, PJ-Bass, Shortscale (EB-0-Bauform), Mustang, Akustikbass, Kontrabass, Höfner Violin Bass und Rickenbacker 4003 mit neun einzeln generierten, transparenten PNGs. Auswahl unter „Bass“, Kontrabass zusätzlich unter „Streicher“ und im Orchester-Editor. Vorhandene Bassmaße und gespeicherte Routingdaten bleiben erhalten. [Assets, Prompts und Herstellerreferenzen](stageplot-assets/objects/bass-instruments-v1.json).

- **E-Gitarren und weitere Saiteninstrumente:** Vier bestehende E-Gitarrenbilder ersetzt; elf Modelle unter „E-Gitarre“: Strat, Telecaster, Jazzmaster, Jaguar, Les Paul, SG, ES-335, Explorer, Flying V, King V und Superstrat. Mandoline, Lap Steel und Ukulele sind eigene Bausteine. Alle 14 Bilder wurden einzeln mit imagegen erstellt und lokal als transparente PNGs eingebunden. [Dateien, Prompts und Herstellerreferenzen](stageplot-assets/objects/electric-instruments-v1.json).
- **Gitarren und Banjo als Bildassets:** Dreadnought, Cutaway, Klassik, Gypsy, Jazzgitarre und Banjo verwenden sechs separat generierte, transparente PNGs. Fünf Gitarrenmodelle unter „Akustikgitarre“; Banjo als eigener Baustein; bestehende Typen, Winkel und Routingdaten bleiben erhalten. [Assets, Prompts und Herstellerreferenzen](stageplot-assets/objects/acoustic-instruments-v1.json).
...
- **Instrumente und Routing:** Wählbare Treppenstufen, MD 421 Kompakt, mehrzeilige freie Notizen, Akustikgitarrenmodelle und direkte Eigenschaften einzelner Streicher. Dual Mono auch für Amps, Pedalboards und einzelne Streicher; weitere Geräteausgänge bleiben erhalten. Es erzeugt zwei unabhängige Abnahmen; eine DI-Box lässt sich einem vorhandenen Signal zuordnen. Die Routingliste zeigt Instrument, Abnahme, Stagebox und Pultkanal; Sortieren ändert keine Kanalnummern.
- **Drums & Percussion:** Zoombare und verschiebbare Arbeitsfläche; der tatsächliche Aufbau bestimmt die Größe auf der Bühne. [Generiertes Vocal-Boom-Asset und Prompt](stageplot-assets/percussion/vocal-boom-top-v1.json). Gemeinsame Vorlagen und frei kombinierbare Teile einschließlich Vocal-Mikrofon mit Boom und Cajon/Kick/Crash. Cajon-Abnahme vorne, hinten oder beidseitig mit eigener Mikrofonwahl. [Cajon-Asset und Prompt](stageplot-assets/percussion/cajon-top-v1.json).
...
- **Bewegliche Beschriftungen:** Kleine, linksbündige Karten mit grünem Akzent und dezentem Schatten in heller und dunkler Darstellung. Direkt am Schild ziehen, fokussiert mit Pfeiltasten um 2 cm (Shift: 10 cm) verschieben und mit Pos1 zurücksetzen. Objekte bleiben dabei an ihrem Platz. Relative Positionen bleiben in lokalen Entwürfen, Projektdateien, Offline-Links und Exporten erhalten; Online-Freigaben benötigen Migration 0006.
...
- **Projektübersicht am Handy:** „Projekt hinzufügen“ steht direkt oben vor Suche und Projekten. Die Karten zeigen eine große echte Bühnenvorschau im dunklen Rahmen und einen klaren Lime-Button zum Öffnen. Maße, Bausteinanzahl, Speicherstatus und kopierbare ID bleiben sichtbar. Öffnen, Projektdaten, Teilen, Sicherung und Löschen sind direkt erreichbar.
...
- **Objekteigenschaften:** Kompakte, direkt eingebbare und ziehbare Positions- und Größenwerte. Kleiner Drehknopf mit 0°-Reset, ±45°-/±90°-Tasten, Grad-Eingabe und Tastaturbedienung. Vergrößern/Verkleinern ändert die Maße um den Faktor 1,1; bei mehrteiligen Aufbauten ändern sich die Abstände, die physischen Instrumentmaße bleiben erhalten. Modellwechsel steht im Objektkopf, Keyboard-Ausgänge separat, der Drum Designer ist direkt erreichbar. Routing, Strom/Funk und Notizen sind aufklappbar. Jede Ziehgeste ist ein Rückgängig-Schritt; Escape und abgebrochene Kontakte stellen den vorherigen Stand wieder her. Helle/dunkle Darstellung, schmale Ansichten und lokale Speicherung bleiben erhalten.
...
- **Illustrierte Bühnentreppen:** lokales Bildgenerierungs-Asset mit einzeln aufgebauten Stufen. Breite, Tiefe und Stufenanzahl bleiben unabhängig einstellbar; Metallkanten behalten ihre Proportionen und tiefer liegende Stufen werden sanft dunkler. Dieselbe Darstellung gilt für Bühnenrand, freie Treppenobjekte, Hausgrundrisse, Projektvorschauen und Export.
