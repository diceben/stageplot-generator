...
const StageplotMicPicker=(()=>{
  ...
  function mount(host,options){
    ...
    host.innerHTML='<div class="sp-mic-current"></div><div class="sp-mic-custom" hidden><label>Eigenes Mikrofonmodell<input maxlength="80" placeholder="Hersteller und Modell"></label><button type="button" data-mic-custom-use>Modell verwenden</button></div><label class="sp-mic-search" for="'+id+'"><span>Mikrofon suchen</span><div><input id="'+id+'" type="search" placeholder="Modell, Hersteller oder Bauart …" autocomplete="off" spellcheck="false" aria-describedby="'+id+'-status"><button type="button" data-mic-search-clear aria-label="Mikrofonsuche leeren" hidden>×</button></div></label><div class="sp-mic-filters" role="group" aria-label="Mikrofon-Schnellzugriffe"></div><div class="sp-mic-manufacturers"><button type="button" data-mic-scroll="-1" aria-label="Vorherige Hersteller">‹</button><div class="sp-mic-brands" role="group" aria-label="Mikrofon-Hersteller"></div><button type="button" data-mic-scroll="1" aria-label="Weitere Hersteller">›</button></div><p class="sp-mic-status" id="'+id+'-status" role="status" aria-live="polite"></p><div class="sp-mic-results" role="group" aria-label="Mikrofonmodelle"></div><small class="sp-mic-photo-credit"><a href="./stageplot-assets/mics/ATTRIBUTION.md" target="_blank" rel="noopener">Bildquellen</a></small>';
