...
const ctx={objectSize:o=>({w:o.width,d:o.depth}),stairStates:s=>(s.steps||[]).map((record,id)=>({record,id})),stairsGeometry:(s,id)=>s.steps[id],iemRect:s=>s.iemZone||null,compiledVenue:s=>({bounds:s.bounds}),mobileWorkspace:()=>false,editorFit:null};
