-- Inventory shares the account owner policy and optimistic revision checks.
alter table public.stageplot_documents drop constraint stageplot_documents_kind_check;
alter table public.stageplot_documents add constraint stageplot_documents_kind_check check (kind in ('project','drum_template','stage_template','inventory'));
alter table public.stageplot_documents drop constraint stageplot_documents_id_check;
alter table public.stageplot_documents add constraint stageplot_documents_id_check check (id ~ '^(setup|drum|stage-template|inventory)-[a-z0-9-]+$');
...
begin
  ...
  if p_kind not in ('project', 'drum_template', 'stage_template', 'inventory') then raise exception 'INVALID_KIND'; end if;
  if p_id !~ '^(setup|drum|stage-template|inventory)-[a-z0-9-]+$' then raise exception 'INVALID_ID'; end if;
...
begin
  if v_user is null then raise insufficient_privilege using message = 'AUTH_REQUIRED'; end if;
  if p_kind is not null and p_kind not in ('project', 'drum_template', 'stage_template', 'inventory') then raise exception 'INVALID_KIND'; end if;
