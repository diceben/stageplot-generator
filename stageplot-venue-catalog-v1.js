...
(function(root,factory){
  ...
  function open(options){
    ...
    dialog.innerHTML='<header class="svc-head"><div><span class="svc-beta">BETA · KOSTENLOS</span><h2 id="svc-title">Bühnen finden</h2><p>Reale Locations für deinen nächsten Stageplot. Frei zugänglich, ohne Account.</p></div><button type="button" data-close aria-label="Bühnenkatalog schließen">×</button></header>'+
      '<div class="svc-searchbar"><label class="svc-search"><span aria-hidden="true">⌕</span><input type="search" placeholder="Location, Stadt oder Bühne suchen …" aria-label="Bühnen suchen" autocomplete="off" autofocus></label><div class="svc-filters" aria-label="Art der Location">'+['Alle','Club','Theater'].map(type=>'<button type="button" data-filter="'+type+'" aria-pressed="'+(type==='Alle')+'">'+type+'</button>').join('')+'</div></div>'+
