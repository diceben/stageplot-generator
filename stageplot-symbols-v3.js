...
const STAGEPLOT_TECH_TOP_VIEWS=Object.freeze({
  ...
  "mic-boom":{"width":1450,"height":1450,"file":"mic-boom-arm-top-v3.webp","rect":[702.9493,200.5074,76.6526,745],"part":"mic-boom"},
...
function stageplotTechFrame(type,options={}) {
  if(type==='mic')return options.stand==='round'?{width:250,height:250}:{width:1450,height:1450};
...
// Fixed head directions are stage-relative; the stand still rotates as one object.
function stageplotMicLayout(options={}) {
  const direction=['left','right'].includes(options.boomDirection)?options.boomDirection:'up',projection=direction==='up'?.34:1;
  const heading=['up','right','down','left'].includes(options.micHeadDirection)?{up:0,right:90,down:180,left:270}[options.micHeadDirection]:undefined;
  const headAngle=heading===undefined?0:((heading-(Number(options.angle)||0))%360+360)%360;
  const tip=options.stand==='round'?[125,125]:direction==='left'?[200.5074,730.2502]:direction==='right'?[1249.4926,719.7498]:[719.7498,725+(200.5074-725)*projection];
  return {direction,projection,armAngle:direction==='left'?-90:direction==='right'?90:0,tip,headAngle,headRect:options.stand==='round'?[-33.2336,-115,66.4671,230]:STAGEPLOT_TECH_TOP_VIEWS['mic-boom-head'].rect};
}
function stageplotMicVisibleBounds(options={}) {
  const layout=stageplotMicLayout(options),points=[],rotate=([x,y],angle)=>{const r=angle*Math.PI/180;return [x*Math.cos(r)-y*Math.sin(r),x*Math.sin(r)+y*Math.cos(r)];};
  const rect=(box,transform)=>{const [x,y,w,h]=box;for(const p of [[x,y],[x+w,y],[x+w,y+h],[x,y+h]])points.push(transform(p));};
  if(options.stand==='round'){if(options.micLayer!=='upper')rect([0,0,250,250],p=>p);}
  else {
    if(options.micLayer!=='upper')rect(STAGEPLOT_TECH_TOP_VIEWS['mic-tripod-base'].rect,([x,y])=>[x+400,y+400]);
    if(options.micLayer!=='base')rect(STAGEPLOT_TECH_TOP_VIEWS['mic-boom'].rect,([x,y])=>{const p=rotate([x-725,(y-725)*layout.projection],layout.armAngle);return [p[0]+725,p[1]+725];});
  }
  if(options.micLayer!=='base')rect(layout.headRect,p=>{const q=rotate(p,layout.headAngle);return [q[0]+layout.tip[0],q[1]+layout.tip[1]];});
...
function stageplotTechArtwork(type,options={}) {
  ...
  if(type==='drum-throne')art=stageplotTechThrone();
    ...
    const {direction,projection,armAngle,tip,headAngle,headRect}=stageplotMicLayout(options);
    const head='<g data-part="'+(options.stand==='round'?'round-base-microphone':'boom-microphone')+'" data-mic-direction="'+(['up','right','down','left'].includes(options.micHeadDirection)?options.micHeadDirection:'up')+'" data-display-enlarged="true" transform="translate('+tip[0]+' '+tip[1]+')'+(headAngle?' rotate('+headAngle+')':'')+'">'+stageplotTechImage('mic-boom-head',headRect)+'</g>';
    const base=options.stand==='round'?stageplotTechImage('mic-round-base'):'<g transform="translate(400 400)" data-part="stand-foot-circle" data-diameter-mm="600">'+stageplotTechImage('mic-tripod-base')+'</g>';
    const upper=(options.stand==='round'?'':'<g data-part="boom-arm-'+direction+'" transform="rotate('+armAngle+' 725 725) translate(725 725) scale(1 '+projection+') translate(-725 -725)">'+stageplotTechImage('mic-boom')+'</g>')+head;
    art=options.micLayer==='base'?base:options.micLayer==='upper'?upper:base+upper;
...
// Perspective lighting drawings share the saved physical footprint, but keep
// their native proportions. They are not measured overhead silhouettes.
const STAGEPLOT_LIGHT_VIEWS=Object.freeze({
  wash:{width:350,height:350},
  'light-par':{width:300,height:300},
  'light-moving-spot':{width:350,height:280},
  'light-spark':{width:250,height:210},
  'light-moving-wash':{width:420,height:320},
  'light-flightcase':{width:800,height:600},
  'light-fog':{width:590,height:330},
  'light-wave-bar':{width:1120,height:240},
  'light-bar':{width:1000,height:120}
});
...
function createStageplotSymbolV3(type, options = {}) {
  ...
  if(lighting){
    ...
    return '<g data-equipment="'+type+'" data-lighting-view="'+(type==='light-flightcase'?'orthographic-top':'elevated-front')+'"><rect width="'+width+'" height="'+height+'" fill="none" stroke="none" pointer-events="none" data-metric-frame="true"/><image href="stageplot-assets/lights/'+type+'-illustrated-v1.webp" width="'+width+'" height="'+height+'" preserveAspectRatio="xMidYMid meet" data-rendered-lighting-asset="'+type+'"/></g>';
  }
  ...
  }else if(['guitar','guitar-tele' ,'guitar-les-paul','guitar-es','bass','bass-j'].includes(type)){
