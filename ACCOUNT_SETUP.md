...
8. [0008_routing_devices.sql](supabase/migrations/0008_routing_devices.sql)

Nur noch fehlende Migrationen ergänzen. Migration 0008 erhält physische DI-Boxen mit ihrer Kanalbelegung, zusätzliche Abnahmen und Monitoring-Geräte in Online-Freigaben. Bereits freigegebene Pläne erhalten diese Angaben beim nächsten Aktualisieren des Links. Private Notizen, Kontakte und Inventarverweise bleiben ausgeschlossen; Berechtigungen bleiben unverändert. Lokal funktionieren die Routing-Einstellungen auch ohne Datenbank. Migration 0007 erhält Stufenanzahl, zusätzliche Drum-/Percussion-Teile und deren Mikrofonwahl. Migration 0006 erhält die verschobenen Positionen von Objektbeschriftungen in Freigaben. Migration 0005 beschränkt anonyme Browser-Sitzungen auf die Freigabe-Funktionen. Private Dokumente und der spätere Gerätesync benötigen eine dauerhafte Anmeldung; die Eigentümerprüfung bleibt zusätzlich bestehen. Die private Dokumenttabelle speichert Projekte, Drumvorlagen, Bühnenvorlagen und Inventareinträge. Row Level Security begrenzt den Zugriff auf den angemeldeten Eigentümer. Der Geräteabgleich verwendet ausschließlich `stageplot_sync_push` und `stageplot_sync_pull`.
