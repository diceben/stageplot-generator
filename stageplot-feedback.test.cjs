...
const start=html.indexOf('  const footprintTouches='),end=html.indexOf("  window.addEventListener('pointermove',moveObjectDrag);",start);vm.runInContext(extract('stageObjectTarget')+'\n'+html.slice(start,end),pc);
const event=(id,x,y=0)=>({target:{closest:selector=>selector==='[data-object],[data-mic-object]'?{dataset:{object:'station-1'}}:null},pointerType:'touch',pointerId:id,clientX:x,clientY:y,preventDefault(){},stopImmediatePropagation(){}});
