...
function createStageplotOrchestraModel() {
  ...
  function normalize(value){
    ...
    for(const [i,p]of value.parts.slice(0,180).entries()){
      ...
      parts.push({...part(c.id,id,0,0,section),x:round(finite(p.x,0,-25,25)),y:round(finite(p.y,0,-25,25)),angle:round((finite(p.angle,0,-36000,36000)%360+360)%360),label:clean(p.label??c.name),enabled:p.enabled!==false,pickup:['mic','dual'].includes(p.pickup)?p.pickup:'none',...(Number.isFinite(p.width)?{width:round(finite(p.width,c.w,.02,5))}:{}),...(Number.isFinite(p.depth)?{depth:round(finite(p.depth,c.d,.02,5))}:{})});
    }
