...
const physical=ctx.stage.routing.devices.find(device=>device.objectId==='box');assert.equal(physical.modelId,'radial-j48');assert.equal(physical.channels,1);
...
const remap=new Map([['instrument','station-1'],['box','station-2'],['bass','station-3']]);ctx.stage.routing=ctx.normalizeRouting(clone(ctx.stage.routing),remap);ctx.objects=ctx.objects.map(o=>({...o,id:remap.get(o.id)}));ctx.syncRoutingFromStage(false,false);
assert.equal(ctx.stage.routing.inputs.length,3,'Normalize/remap + sync keeps two DI users and one extra pickup without recreating the DI placeholder.');
assert(ctx.stage.routing.disabledSources.includes('station-2:io-out-1'));assert.equal(ctx.audioObjectRows(ctx.objects[1]).length,2);
assert(ctx.stage.routing.devices.some(device=>device.objectId==='station-2'));assert(ctx.stage.routing.inputs.some(row=>row.sourceKey==='station-1:pickup-extra-mic'));
ctx.StageplotRoutingModel.removeInput(ctx.stage.routing,extra.id);ctx.syncRoutingFromStage(false,false);assert.equal(ctx.stage.routing.inputs.length,2,'Removing a manually added pickup remains removed on reconciliation.');
const diInspector={innerHTML:'',hidden:false};const oldDollar=ctx.$;ctx.$=()=>diInspector;ctx.renderObjectAudio(ctx.objects[1]);ctx.$=oldDollar;assert(!diInspector.innerHTML.includes('data-audio-use='),'DI inspector cannot re-enable a separate generated placeholder signal.');
