...
vm.runInContext(['parseIoValue','defaultObjectIo','normalizeObjectIo','objectOutputPortKey','objectOutputBaseName','objectOutputSignal','generatedInputSpecs','generatedOutputSpecs','projectIdentity','normalizedObjectDimensions','editableObjectSize','normalizeSetupDocument'].map(extract).join('\n'),ctx);
...
for(const entry of installed){
  ...
  assert(c.vb.every((n,i)=>Math.abs(n-[c.w,c.d][i]*100)<1e-6),entry.id+' metric viewBox mismatch');
...
const orchestra=require('./stageplot-orchestra-v1.js')();
const updated=manifest.objects.filter(o=>o.integration==='orchestra');
assert.equal(updated.length,5);
for(const entry of updated){
  const image=orchestra.imageMarkup(orchestra.part(entry.id));
  assert(image.includes('./stageplot-assets/objects/'+entry.id+'-v1.webp'));
  assert(image.includes('preserveAspectRatio="xMidYMid meet"'));
  assert.equal(orchestra.byId[entry.id].w,entry.w);assert.equal(orchestra.byId[entry.id].d,entry.d);
}
...
Object.assign(ctx,{URL,window:{location:{href:'https://stageplot.test/',origin:'https://stageplot.test'}},exportImageCache:new Map(),
  ...
  FileReader:class{readAsDataURL(bytes){this.result='data:image/webp;base64,'+bytes.toString('base64');this.onload();}},
  Image:class{constructor(){throw new Error('Compressed WebP unnecessarily rasterized.');}}});
vm.runInContext(extract('blobDataUrl')+'\n'+extract('localImageDataUrl'),ctx);
