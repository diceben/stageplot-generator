...
(function(global){
  ...
  function create({client,storage=global.localStorage,ownerId,scope='',onConflict=()=>{}}){
    ...
    function read(){const raw=storage.getItem(key);if(!raw)return {version:2,pending:[],revisions:{},synced:{},confirmed:{}};const state=JSON.parse(raw);if(state.version!==2||!Array.isArray(state.pending)||!state.revisions||!state.synced)throw new Error('Cloud-Warteschlange nicht lesbar.');state.confirmed=state.confirmed||{};return state;}
      ...
      while(state.pending.length){
        ...
        if(data.status==='conflict'){
          ...
          if(!op.deleted){const id=kinds[op.kind]+'-'+Date.now().toString(36)+'-'+Math.random().toString(36).slice(2,10),limit=op.kind==='stage_template'?40:60,entry={...clone(op.entry),id,name:op.entry.name.slice(0,limit-16)+' · Konfliktkopie'};
            if(op.kind==='project'){entry.document.stage.title=entry.name;entry.document.stage.projectId='SP-'+id.slice(6).toUpperCase();if(entry.document.stage.project)entry.document.stage.project.name=entry.name;}
          ...
          state.pending.shift();state.synced[k]=op.deleted?null:fingerprint(op.entry);delete state.confirmed[k];write(state);continue;
        }
        if(!['saved','deleted'].includes(data.status)||!Number.isFinite(Number(data.revision)))throw new Error('Ungültige Sync-Bestätigung.');
        state.revisions[k]=Number(data.revision);state.confirmed[k]=op.deleted?null:state.revisions[k];state.synced[k]=op.deleted?null:fingerprint(op.entry);state.pending.shift();write(state);completed++;
      }
      return {completed,copies,pending:state.pending.length};
    });}
    async function accept(kind,row){return lock(()=>{const state=read(),k=recordKey(kind,row.id);if(state.pending.some(op=>op.kind===kind&&op.id===row.id))return false;state.revisions[k]=Number(row.revision)||0;state.confirmed[k]=row.deleted?null:state.revisions[k];state.synced[k]=row.deleted?null:fingerprint(row);write(state);return true;});}
    async function pull(kind,{acknowledge=true}={}){if(!kinds[kind])throw new Error('Unbekannte Datenart.');return lock(async()=>{
      ...
      for(;;){
        ...
        for(const row of page.records){
          ...
          if(acknowledge){state.revisions[k]=Number(row.revision)||0;state.confirmed[k]=row.deleted?null:state.revisions[k];state.synced[k]=row.deleted?null:fingerprint(entry);}records.push({...entry,revision:Number(row.revision)||0,deleted:row.deleted===true});
        }
    ...
    return {projects:repo('project'),drumTemplates:repo('drum_template'),stageTemplates:repo('stage_template'),inventory:repo('inventory'),save,remove,pull,accept,flush,
      local:{state:()=>clone(read()),matches:(kind,entry)=>read().synced[recordKey(kind,entry.id)]===fingerprint(entry),confirmed:(kind,entry)=>{const state=read(),k=recordKey(kind,entry.id);return !!state.confirmed[k]&&state.confirmed[k]===state.revisions[k]&&state.synced[k]===fingerprint(entry)&&!state.pending.some(op=>op.kind===kind&&op.id===entry.id);}},ownerId};
  }
