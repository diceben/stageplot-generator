...
for(const c of ctx.catalog.filter(c=>!c.orchestraPart&&!c.percussionPart&&!['orchestra','percussion','drums','text'].includes(c.id))){
 ...
 for(const scale of [2,37,180]){
  ...
  const frame=ctx.stageplotTechFrame(c.art||c.id,{type:c.id})||ctx.stageplotLightingFrame(c.art||c.id)||ctx.stageplotAcousticFrame(c.art||c.id)||ctx.stageplotStringFrame(c.art||c.id)||ctx.stageplotBassFrame(c.art||c.id);
  if(frame){
