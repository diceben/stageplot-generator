...
assert.deepEqual(JSON.parse(JSON.stringify(drumCatalog.map(item=>({id:item.id,name:item.name,short:item.short})))),[{id:'drums',name:'Drums & Percussion',short:'Schlagzeug'}]);
