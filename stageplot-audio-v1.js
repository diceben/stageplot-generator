...
function audioDiControls(o){
  ...
  const selector='<div class="sp-choice-pills" role="group" aria-label="DI-Modell">'+models.map(model=>'<button type="button" data-di-model="'+model.id+'" aria-pressed="'+(device?.modelId===model.id)+'"'+off+'>'+esc(model.name)+'</button>').join('')+'</div>';
  ...
  return '<div class="sp-direct-field"><span>DI-Box</span>'+selector+custom+'<small>'+esc(device?(device.active?'Aktiv':'Passiv')+(device.phantom?' · 48 V':''):'Modell auswählen')+'</small><span>Eingänge</span>'+sockets+'<span>Eingang '+selectedPort+'</span>'+assignment+'</div>';
}
