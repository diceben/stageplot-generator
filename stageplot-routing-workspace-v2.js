(function (global) {
  ...
  function create(host, api) {
    ...
    function pickupCard(row) {
      ...
      const picture = kind === 'Mic' ? micIcon(row.microphone) : kind === 'DI' ? icon('di') : icon(object(sourceId(row)) || 'laptop');
      let editor = '';
