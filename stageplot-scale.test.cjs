...
const previewContext={stageObjectOrder:x=>x,stageArtworkLayers:x=>x.map(object=>({object})),objectCatalog:()=>c,objectSize:()=>tiny,artId:()=> 'tiny',artBoundsCache:new Map([['tiny',{x:2,y:3,width:6,height:4}]]),drumModel:context.drums};vm.createContext(previewContext);
