...
assert.match(html,/id="sp-drum-hover-open"[^>]*><span>Drums &amp; Percussion<\/span><strong>Aufbau bearbeiten<\/strong>/,'Der schwebende Drum-Designer-Schnellzugriff fehlt.');
assert.match(html,/drumButton\.hidden=hidden\|\|!o\|\|\(!drumModel\.isDrums\(o\.type\)&&!\['percussion','orchestra','laptop','rack'\]\.includes\(o\.type\)\)/,'Der Schnellzugriff wird nicht auf ausgewählte Drum-, Percussion-, Orchester-, Playback- und IEM-Objekte begrenzt.');
